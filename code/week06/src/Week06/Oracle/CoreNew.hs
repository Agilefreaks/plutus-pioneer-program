...
module Week06.Oracle.CoreNew where
...
import           Ledger                    (Address, Datum (Datum), DatumHash,
                                            PubKeyHash,
                                            ...
                                            getContinuingOutputs, pubKeyHash,
                                            scriptAddress, txId, txOutDatum,
                                            txSignedBy)
import           Ledger.Ada                as Ada (lovelaceValueOf)
import           Ledger.Constraints        as Constraints (mustPayToTheScript,
                                                           mustSpendScriptOutput,
                                                           otherScript,
                                                           typedValidatorLookups,
                                                           unspentOutputs)
import           Ledger.Scripts            (Redeemer (Redeemer))
...
import           Plutus.Contract           as Contract (Contract, Endpoint,
                                                        ...
                                                        tell, utxoAt)
import           Plutus.Contracts.Currency as Currency (CurrencyError,
                                                        OneShotCurrency,
                                                        currencySymbol,
                                                        mintContract)
import qualified PlutusTx
import           PlutusTx.Prelude          (Bool, Eq ((==)), Integer,
                                            Maybe (..), emptyByteString, isJust,
...
mkOracleValidator oracle x r ctx =
    ...
    traceIfFalse "token missing from the outpu" outputHasToken  &&
    case r of
        Update -> traceIfFalse "operator signature missing" (txSignedBy info $ oOperator oracle) &&
                  traceIfFalse "invalid output datum" validOutputDatum
        Use -> traceIfFalse "oracle value change" (outputDatum == Just x) &&
